...
export const taskRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/tasks', async (request, reply) => {
    ...
    if (taskType === 'image') {
      ...
      const limitError = getReferenceImageLimitError(providerProfile, inputImageCount)
      if (limitError) {
        reply.code(400)
        return { message: limitError }
      }
    }
    ...
    return {
      task: loadSerializedTask(app.db, taskId, {
        ...
        exposeDetailedError: auth.role === 'admin',
  ...
  app.get('/api/tasks', async (request, reply) => {
    ...
    if (hasTextSearch) {
      ...
      items = serializeTaskList(rawTasks, auth.role === 'admin', auth.role === 'admin')
    }
  ...
  app.patch('/api/tasks/:taskId', async (request, reply) => {
    ...
    const serializedTask = serializeTask(task, auth.role === 'admin', auth.role === 'admin')
  ...
  app.get('/api/tasks/events', async (request, reply) => {
    ...
    const unsubscribe = app.taskEvents.subscribeAll((event: TaskListEventRecord) => {
      ...
      const task = serializeTask(taskRecord, auth.role === 'admin', auth.role === 'admin')
