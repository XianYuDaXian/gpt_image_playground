...
export function isMultiModelImageMode(apiMode: ApiMode | null | undefined): boolean {
  return apiMode === 'venice_images' || apiMode === 'wavespeed' || apiMode === 'kie'
}

/** 按端点类型返回默认参考图上限 */
export function defaultMaxReferenceImages(apiMode: ApiMode | null | undefined): number {
  return isMultiModelImageMode(apiMode) ? 3 : MAX_REFERENCE_IMAGES_HARD_LIMIT
}

/** 规范化管理员配置的参考图上限 */
export function normalizeMaxReferenceImages(
  apiMode: ApiMode | null | undefined,
  value: unknown,
): number {
